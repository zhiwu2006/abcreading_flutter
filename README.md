...
- **课程编辑器** (v0.3 新增):
  - **课程管理**: 完整的课程增删改查功能，支持批量操作
  - **多选编辑**: 支持多选模式，可批量删除和导出课程
  - **详情编辑**: 点击课程进入详细编辑页面，支持编辑所有字段
  - **数据同步**: 本地编辑优先，支持手动同步到数据库
  - **导入导出**: 支持JSON格式的课程数据导入导出
...
### 单词列表优化
- **界面简化**：
  - AppBar只保留信息按钮和测试按钮，移除冗余的定位和清除按钮
  - 简化用户交互流程，减少视觉干扰
  - 保持核心功能完整的同时提升界面清洁度

- **性能大幅提升**：
  - 滚动响应速度提升80%（延迟从500ms降至100ms）
  - 动画流畅度提升40%（动画时长从1000ms优化至600ms）
  - 简化滚动位置计算算法，移除复杂的二次精确定位逻辑
  - 代码量减少62%（从124行优化至47行）

- **用户体验优化**：
  - 移除大量调试输出，减少控制台负担
  - 移除点击时的SnackBar反馈，提升操作流畅度
  - 优化ListView缓存和物理特性，滚动更流畅
  - 保持所有核心功能：自动定位、位置记忆、横竖屏适配

### 技术改进
- **内存优化**：减少不必要的UI更新和状态变化
- **算法优化**：使用更高效的滚动计算和动画曲线
- **代码质量**：简化逻辑，提高可维护性

## 0.4.1 更新内容

- **长按释义体验**
  - 长按开始即触发（onLongPressStart）：单词瞬时高亮并弹出释义弹窗，关闭后自动还原。
  - 单实例弹窗：防止重复弹出或叠加。
- **词典来源精简**
  - 仅保留英英释义，使用 Free Dictionary（dictionaryapi.dev）。
  - 移除本地词库、有道接口以及翻译兜底逻辑；未命中时不再弹出"在线查词"提示。
- **依赖与配置**
  - 词典仅需 `http` 依赖，无需密钥和本地词库。

使用提示：故事阅读界面长按单词即可查看英英解释与音标，未命中则无提示（避免打扰）。
...
- **数据同步机制**：
  - 本地编辑优先策略，提供快速响应
  - 手动同步控制，避免频繁网络请求
  - 双向同步支持：上传本地到数据库 / 从数据库下载
  - 同步状态实时反馈，操作结果清晰明确

### 用户体验优化
- **界面布局改进**：课程列表前置，导入功能后置，符合使用习惯
- **操作提示系统**：智能提示用户何时需要同步数据库
- **状态管理优化**：精确跟踪数据变化，提供即时视觉反馈
- **错误处理完善**：详细的错误信息和异常处理机制

### 技术架构升级
- **服务层增强**：LessonManagerService 新增课程管理方法
- **本地缓存优化**：改进 SharedPreferences 数据管理
- **内存管理**：正确的控制器生命周期管理
- **代码结构**：清晰的分层架构，便于维护和扩展
